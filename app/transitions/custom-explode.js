...
export default function customExplode(...pieces) {
    ...
    var promises = pieces.map((piece) => {
        if (piece.matchBy) {
            ...
            return splitAndExplode(this, piece, seenElements);
        } else {
...
function splitAndExplode(context, piece, seen) {
    ...
    if (!(context.oldElement && piece.pickOld)) {
        ...
        promises.pushObject(Promise.resolve());

    } else {

        // first it should use the pickOld selector to find elements
        var hits = Ember.A(context.oldElement.find(piece.pickOld).toArray());
        ...
        hits.forEach((elt) => {
            var attrValue = Ember.$(elt).attr(piece.unique);
            var pick = selector(attrValue);

            promises.pushObject(
                explodePiece(context, {
                    pick: pick,
                    use: piece.use
                }, seen)
            );
    ...
    if (!(context.newElement && piece.pickNew)) {
        ...
        var hits = Ember.A(context.newElement.find(piece.pickNew).toArray());
